...
public:
  template <typename TProcessorStates, typename TBus, typename... Args,
            typename = std::enable_if_t<(std::is_same_v<Args, uint32_t> && ...)>>
  static Result<std::array<u32, sizeof...(Args)>> ReadMemory(TProcessorStates &pstates, TBus &bus,
                                                             Args... value) {
    using TElement = typename std::tuple_element<0, std::tuple<Args...>>::type;
    std::array<TElement, sizeof...(Args)> result_ma;
    auto sc = AssignReadResult(pstates, bus, result_ma, value...);
  ...
  template <typename TProcessorStates, typename TBus>
  static Result<void> CpyFromEmuMem(TProcessorStates &pstates, TBus &bus, char *dest_ptr,
  ...
  template <typename TProcessorStates, typename TBus>
  static Result<u32> CpyToEmuMem(TProcessorStates &pstates, TBus &bus, me_adr_t dest_ptr,
...
private:
  ...
  template <std::size_t I = 0, std::size_t N, typename TProcessorStates, typename TBus, typename T,
            typename... Args>
      static typename std::enable_if <
      I<(N - 1), StatusCode>::type AssignReadResult(TProcessorStates &pstates, TBus &bus,
                                                    std::array<uint32_t, N> &result_ma, T value,
                                                    Args... args) {
    auto r_w1 = bus.template Read<u32>(pstates, value);
    ...
    auto sc = AssignReadResult<I + 1, N, TProcessorStates, TBus, Args...>(pstates, bus, result_ma,
                                                                          args...);
    return sc;
  }
  ...
  AssignReadResult(TProcessorStates &pstates, TBus &bus, std::array<uint32_t, N> &result_ma,
                   T value) {
    auto r_w1 = bus.template Read<u32>(pstates, value);
    // abort if an error occured
    if (r_w1.IsErr()) {
